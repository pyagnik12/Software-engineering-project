...
filterOprions = st.multiselect("Select the filter category",
                        options=["catagory", "county", 
                        "Business name"])


for i in range(len(filterOprions)):
    changed_profile_value = st.text_input(f"Change {filterOprions[i]}", key=i)
